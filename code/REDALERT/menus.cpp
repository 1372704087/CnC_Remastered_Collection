...
int Main_Menu(unsigned long )
{
	...
	*/
	...
	int	d_dialog_h = 120 * RESFACTOR;

	int	d_dialog_x = ((SeenBuff.Get_Width() - d_dialog_w) / 2);
	int	d_dialog_y = ((SeenBuff.Get_Height() - d_dialog_h) / 2);

	int d_button_w = 200 * RESFACTOR;
	int d_button_h = 20 * RESFACTOR;
	int d_button_dy = d_button_h + 8 * RESFACTOR;		// spacing between top of two buttons
...
// jmarshall end
	...
	int version_x = 325;
	int version_y = SeenBuff.Get_Height() - 20;
